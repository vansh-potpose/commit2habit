...
const Challenges = ({ setCurrentAbility, currentAbility, deleteChallenge, challenges, changeCompletedChallenges, updateChallenge, addChallenge }) => {
    return (
        <div className='bg-black p-5 px-10 border-l border-borderColor min-h-[calc(100vh-71px)]' >
            <h1 className='font-semibold text-2xl flex justify-between items-center my-3'>
                <div className='group flex gap-4'>Challenges

                    <button onClick={() => { addChallenge(currentAbility.name) }} className='group-hover:opacity-100 opacity-0  ease-in-out duration-500 bg-green-500 text-white text-lg p-1  rounded-full'>
                ...
                <button onClick={() => setCurrentAbility({})} className=' bg-red-500 text-white text-lg px-3 py-1 rounded-md'>Exit</button>
            </h1>
